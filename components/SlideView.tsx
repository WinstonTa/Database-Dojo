...
/** Wide diagrams get their own full-width row instead of the side column. */
function isWide(spec: DiagramSpec): boolean {
  if (spec.kind === "class-model") {
    return (
      spec.cols >= 3 ||
      spec.associations.some((a) => a.associationClass) ||
      spec.entities.length >= 4
    );
  }
  if (spec.kind === "object-graph") return spec.cols >= 3 || spec.nodes.length >= 5;
  return false;
}
  ...
  return (
    <div className="flex flex-col gap-4">
      {body.map((p, i) => (
        <p
          key={i}
          className="font-body text-[1.02rem] leading-relaxed text-[var(--ink)]"
        >
          {p}
        </p>
      ))}
...
function Aside({ label, text }: { label: string; text: string }) {
  return (
    <aside className="mt-6 border-l-2 border-[var(--accent)] bg-[var(--accent-soft)] px-4 py-3">
      <p className="eyebrow mb-1 !text-[var(--accent)]">{label}</p>
...
function ConceptView({ slide }: { slide: ConceptSlide }) {
  const wide = slide.diagram ? isWide(slide.diagram) : false;
  const heading = (
    <>
      <p className="eyebrow mb-3">{slide.eyebrow ?? slide.articleLabel}</p>
...
function ExerciseView({
  ...
  const text = (
    <div className="flex flex-col">
      <p className="eyebrow mb-3">
        {slide.eyebrow ?? `Exercise · ${slide.articleLabel}`}
      </p>
      <h2 className="mb-5 text-[clamp(1.6rem,3.4vw,2.3rem)] text-[var(--ink)]">
        {slide.title}
      </h2>
        ...
        <div className="mt-6 border-l-2 border-[var(--highlight)] bg-[var(--highlight-soft)] px-4 py-3">
          ...
          <ul className="flex flex-col gap-2">
            {slide.solution.notes.map((n, i) => (
              <li
                key={i}
                className="font-body text-[0.93rem] leading-relaxed text-[var(--ink)]"
              >
                {n}
              </li>
            ))}
  ...
  const visual = revealed ? (
    ...
    <button
      type="button"
      onClick={onReveal}
      className="dot-grid flex min-h-[240px] w-full flex-col items-center justify-center gap-3 border border-dashed border-[var(--rule-strong)] bg-[var(--panel)] p-6 text-center transition-colors hover:border-[var(--accent)]"
    >
  ...
  const wide = isWide(slide.solution.diagram);
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-12">
      ...
      <div className="lg:pt-1">{visual}</div>
    </div>
  );
}
